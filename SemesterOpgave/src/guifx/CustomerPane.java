...
public class CustomerPane extends GridPane {
	...
	private ListView<Medlem> lvwCustomers;
	...
	public CustomerPane() {
		...
		ChangeListener<Medlem> listener = (ov, oldCustomer, newCustomer) -> this.selectedCustomerChanged();
	...
	private ArrayList<Medlem> initAllCustomerList() {
		ArrayList<Medlem> list = new ArrayList<>();
		for (Medlem emp : Service.getMedlemmer()) {
			list.add(emp);
		}
		return list;
	}
	...
	private void updateAction() {
		Medlem medlem = lvwCustomers.getSelectionModel().getSelectedItem();
		if (medlem == null) {
			return;
		}

		CustomerWindow dia = new CustomerWindow("Update Customer", medlem);
		dia.showAndWait();
